...
// 批量插入
func (t *Table) Inserts(rows lib.SqlIns, maxTrans int) []int64 {
	return t.adds(rows, "insert into", maxTrans)
}
...
func (t *Table) add(row lib.SqlIn, opBefore, opAfter string) int64 {
	...
	for k, value := range row {
		if !strings.Contains(k, "`") {
			...
			feildsArray = append(feildsArray, k)
		}
		valuesArray = append(valuesArray, "?")
		vars = append(vars, value)
	}
...
// add 增加数据通过操作符
func (t *Table) adds(rows lib.SqlIns, opBefore string, maxTrans int) []int64 {
	if maxTrans > 0 {
	...
	for i, row := range rows {
		...
		if stmtPre==""{
			stmtPre=fmt.Sprintf("%s %s(%s) values", opBefore, t.tableName, strings.Join(feildsArray, ","))
		}
		sql := fmt.Sprintf("(%s)", strings.Join(valuesArray, ","))
		sqls = append(sqls, sql)
		allsqls = append(allsqls, sql)
		if maxTrans > 0 && ((i+1)%maxTrans == 0 || i == len(rows)-1) {
			smt, err := t.db.prepare(stmtPre+strings.Join(sqls, ","))
			if err != nil {
				log.DebugPrint("===========执行批量插入sql失败:%s", err.Error())
				log.DebugPrint("===========错误批量插入sql:%s", t.GetLastSql())
				t.db.Rollback()
				return nil
			}
			result, err := smt.Exec(vars...)
			if err != nil {
				log.DebugPrint("===========执行批量sql失败:%s", err.Error())
				log.DebugPrint("===========错误批量sql:%s", t.GetLastSql())
				return nil
			}
			effects, err := result.LastInsertId()
			if err != nil {
				log.DebugPrint("===========获取批量插入ID失败:%s", err.Error())
				log.DebugPrint("===========错误批量sql:%s", t.GetLastSql())
				return nil
			}
			count := int64(len(sqls))
	...
	if maxTrans > 0 {
		t.sqls(stmtPre,allsqls, allvars...)
		t.db.Commit()
	} else {
		t.sqls(stmtPre,sqls, vars...)
		smt, err := t.db.prepare(stmtPre+strings.Join(sqls, ","))
		if err != nil {
			log.DebugPrint("===========执行批量插入sql失败:%s", err.Error())
			log.DebugPrint("===========错误批量插入sql:%s", t.GetLastSql())
...
// sql
func (t *Table) sqls(stmtPre string,sqls []string, whereParams ...interface{}) {
	t.preSql = stmtPre+strings.Join(sqls, ",")
	t.preParams = whereParams
	t.lastSql = TransSqls(stmtPre,sqls, whereParams...)
}
...
// TransSqls 转原生 sql
func TransSqls(stmtPre string,sqls []string, whereParams ...interface{}) string {
	sql :=stmtPre+ strings.Join(sqls, ",")
	if len(whereParams) > 0 {
		var params []interface{}
		for _, v := range whereParams {
			params = append(params, lib.InterfaceToString(v))
